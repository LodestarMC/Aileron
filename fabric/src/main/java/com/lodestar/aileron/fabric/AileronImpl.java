...
public class AileronImpl implements ModInitializer {
	...
	public static boolean canChargeSmokeStack(@Nullable Player player) {
		...
		return player != null && ElytraItem.isFlyEnabled(Aileron.getElytra(player)) && ((player.getEntityData().get(AileronEntityData.SMOKE_STACK_CHARGES) > 0 && player.isFallFlying()) || player.isCrouching());
	}
